...
def clientthread(conn, addr): 
	...
	#envia un mensaje al cliente cuyo objeto es conn
	nonce = getNonce() 
	list_nonce[addr[0]] = nonce
	print "Mostramos la lista => ",list_nonce
	bienvenida = "Welcome to this server!-"+str(nonce)
	...
	while True: 
			try: 
				...
				if int(separar[2]) == int(list_nonce.get(addr[0])):
					...
					#el inicializamos nonCorrecto como false por lo que no es necesario cambiar el valor.
				...
				if(mac == separar[1]):
					...
					##Similar a como ocurre en nonce

				##Contamos que tipo de mensaje es el que hemos recibido
				##mensaje correcto
...
while True: 
	...
	# crea un hilo por cada usuario que se conecta
